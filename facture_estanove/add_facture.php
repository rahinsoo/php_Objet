...
// Pré-sélection du client si passé en paramètre GET
$client_preselectionne = null;
if (isset($_GET['id']) && !empty($_GET['id'])) {
    $id_client = $_GET['id'];
    
    // Récupérer les infos du client présélectionné
    try {
        $sql = "SELECT nom, prenom FROM CLIENTS WHERE id_client = :id";
        $stmt = $bdd->prepare($sql);
        $stmt->execute(['id' => $id_client]);
        $client_preselectionne = $stmt->fetch(PDO::FETCH_ASSOC);
    } catch(Exception $e) {
        // Si erreur, on ignore simplement
    }
}
...
<main class="container mt-4">
    ...
    <?php if (empty($clients)): ?>
        <div class="alert alert-warning">
            Aucun client disponible. Vous devez d'abord <a href="add_client.php" class="alert-link">ajouter un client</a> avant de créer une facture.
        </div>
    <?php else: ?>
        
        <?php if ($client_preselectionne): ?>
            <div class="alert alert-info">
                <strong>Client présélectionné :</strong> 
                <?php echo htmlspecialchars($client_preselectionne['nom'] . ' ' . $client_preselectionne['prenom']); ?>
            </div>
        <?php endif; ?>
            ...
            <div class="card-body">
                <form action="add_facture.php" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="produits" name="produits" rows="3" required><?php echo htmlspecialchars($produits ?? ''); ?></textarea>
                    </div>
